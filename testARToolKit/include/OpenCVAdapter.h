...
//cv::Matのピクセル値拾得用マクロ(24bit)
#define matB(IMG,X,Y)		(uchar)(IMG).data[(Y)*(IMG).cols + (X)*(IMG).step +0]
#define matG(IMG,X,Y)		(uchar)(IMG).data[(Y)*(IMG).cols + (X)*(IMG).step +1]
#define matR(IMG,X,Y)		(uchar)(IMG).data[(Y)*(IMG).cols + (X)*(IMG).step +2]
#define matGRAY(IMG,X,Y)	(uchar)(IMG).data[(Y)*(IMG).cols + (X)]
